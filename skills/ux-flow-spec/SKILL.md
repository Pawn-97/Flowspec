...
- Render Markdown and handoff artifacts from JSON.
...
2. Extract flows: identify candidate flows.
3. Prepare: compile one selected flow into `flow-prep.json` and `flow-prep.md`.
4. Confirm: wait for designer confirmation of flow prep.
5. Generate: create `ux-flow-spec.json`.
6. Render: create Markdown, traceability, and brief draft from JSON.
7. Validate/review: run deterministic checks and current-agent qualitative review.
8. Revise: auto-fix deterministic low-risk issues only.
9. Handoff: generate prototype brief and prototype agent prompt.
